...
class LabDataDataset(Dataset):
    def __init__(self, data, tokenizer, args, prompt_id='1-1'):
    ...
    def __getitem__(self, idx):
        ...
        if self.task_template['id'] == '1-1':
            source_text = self.task_template['source'].format(mapped_user_id, mapped_item_id)
        elif self.task_template['id'] == '1-2':
            source_text = self.task_template['source'].format(mapped_user_id, item_title)
        elif self.task_template['id'] == '1-6':
            source_text = self.task_template['source'].format(user_desc, mapped_item_id)
        elif self.task_template['id'] == '1-7':
             source_text = self.task_template['source'].format(user_desc, item_title)
...
def main():
    ...
    args.output_dir = os.path.join(project_root, "64_output")
    args.predictions_csv_path = os.path.join(args.output_dir, "64_predictions.csv")
    ...
    args.ft_eval_prompt_id = '1-1' # Prompt to use for prediction of the left-out item
